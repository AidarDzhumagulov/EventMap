...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) (tokenResponse, error) {
	if r.Method != http.MethodPost {
		http.Error(w, "Разрешен только POST метод", http.StatusMethodNotAllowed)
	...
	user, err := h.userRepo.GetByUsername(LoginUser.Username)
	if err != nil {
	...
	if err != nil {
		http.Error(w, "Ошибка при генерации access_token", http.StatusUnauthorized)
		return tokenResponse{}, err
	}

	refreshToken, err := auth.GenerateRefreshToken(user.ID)
	if err != nil {
		http.Error(w, "Ошибка при генерации refresh_token", http.StatusUnauthorized)
		return tokenResponse{}, err
	}
